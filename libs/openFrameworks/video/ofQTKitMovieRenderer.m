...
{
    // Runs in main thread before handing _movie object off to background thread
    self.loading = YES;
    _movie = [QTMovie alloc];
    //ExitMovies();
    //DetachMovieFromCurrentThread([_movie quickTimeMovie]);
    [_movie detachFromCurrentThread];
}

- (BOOL) loadMovieAsync:(NSString*)moviePath pathIsURL:(BOOL)isURL allowTexture:(BOOL)doUseTexture allowPixels:(BOOL)doUsePixels allowAlpha:(BOOL)doUseAlpha
{
    // Thread safety stuff. See: http://developer.apple.com/library/mac/#technotes/tn2125/_index.html
    //EnterMoviesOnThread(0);
    //AttachMovieToCurrentThread([_movie quickTimeMovie]);
	...
	_movie = [_movie initWithAttributes:movieAttributes
										   error: &error];
    ...
    //NSLog(@"Movie duration %lld", movieDuration.timeValue);
